...
export function ProcedureCatalog({
  ...
  const [selectedInstitution, setSelectedInstitution] = useState("Todas las Instituciones");
  ...
  const filteredProcedures = useMemo(() => {
    ...
    return procedures.filter((procedure) => {
      const matchesInstitution =
        selectedInstitution === "Todas las Instituciones" || procedure.institutionName === selectedInstitution;
      const matchesQuery =
        query.length === 0 ||
        [procedure.title, procedure.description, procedure.institutionName, ...procedure.keywords]
          .join(" ")
          .toLowerCase()
          .includes(query);
  ...
  }, [procedures, searchValue, selectedInstitution]);

  const [featured, ...standard] = filteredProcedures;

  return (
    <>
      <SearchHero
        frequentSearches={frequentSearches}
        onFrequentSearch={(value) => setSearchValue(value)}
      ...
      <section className="shell-container grid gap-6 py-10 md:grid-cols-12">
        <aside className="md:col-span-4">
          <div className="institutional-card p-5 md:sticky md:top-24">
            ...
            <label className="mt-5 block text-sm font-bold tracking-[0.05em] text-(--text-main)">
              Institución
              <select
                className="focus-ring mt-2 h-12 w-full border border-(--gold-light) bg-white px-3 font-normal tracking-0"
        ...
        <div className="md:col-span-8">
          <div className="mb-6 flex items-center justify-between gap-4">
            <h2 className="brand-serif text-3xl font-semibold text-(--text-main)">Trámites Disponibles</h2>
          </div>
          {filteredProcedures.length === 0 ? (
            <div className="institutional-card p-8 text-center text-(--text-muted)">
              No se encontraron trámites con esos criterios. Prueba con otra búsqueda o institución.
            </div>
          ) : (
            <>
              {featured ? (
